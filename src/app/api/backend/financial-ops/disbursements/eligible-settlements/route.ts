...
export async function GET(_request: NextRequest) {
  ...
  const response = await fetch(`${getAdminApiBaseUrl()}/admin/financial-ops/disbursements/eligible-settlements`, {
    method: "GET",
    headers: {
  ...
  const payload = await response.json().catch(() => ({
    message: "Unable to load eligible settlements.",
    error: { message: "Unable to load eligible settlements." },
  }));
