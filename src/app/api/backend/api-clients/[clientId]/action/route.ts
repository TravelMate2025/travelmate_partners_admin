...
export async function POST(request: NextRequest, context: Context) {
  ...
  const response = await fetch(`${getAdminApiBaseUrl()}/admin/api-clients/${encodeURIComponent(clientId)}/action`, { method: "POST", headers: { "Content-Type": "application/json", Cookie: `${ADMIN_API_SESSION_COOKIE}=${session.backendSessionKey}` }, body: JSON.stringify(body), cache: "no-store" });
  const payload = await response.json().catch(() => ({ message: "Unable to apply API client action.", error: { message: "Unable to apply API client action." } }));
